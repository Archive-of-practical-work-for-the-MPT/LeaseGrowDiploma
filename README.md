...
Все тестовые аккаунты используют пароль: `adminadmin`

#### 1) Админ

| Роль  | Логин (email)     | Username | Пароль       |
| ----- | ----------------- | -------- | ------------ |
| admin | `admin@gmail.com` | `admin`  | `adminadmin` |

#### 2) Менеджеры

| Роль    | Логин (email)           | Username       | Пароль       |
| ------- | ----------------------- | -------------- | ------------ |
| manager | `ivan.petrov@agro.ru`   | `ivan_manager` | `adminadmin` |
| manager | `manager2@leasegrow.ru` | `manager2`     | `adminadmin` |

#### 3) Пользователи (клиенты)

| Роль   | Логин (email)         | Username          | Пароль       |
| ------ | --------------------- | ----------------- | ------------ |
| client | `client1@agrofarm.ru` | `agrofarm_client` | `adminadmin` |
| client | `client2@zemledel.ru` | `zemledel_client` | `adminadmin` |
| client | `client3@harvest.ru`  | `harvest_client`  | `adminadmin` |
...
Для оплаты в проекте используется тестовая система магазина ЮKassa.

Для проверки оплаты банковскими картами используйте следующие тестовые номера:
...
`CVV` и срок действия карты — любые, главное чтобы год был актуальным.
